...
class NeighborEntry:
    def __init__(self, addr: Addr = None, cost=None, name=None, serialized_entry: str = None):
        if serialized_entry is None:
            self.addr = addr
            self.cost = cost  # None表示链路不可用 X  ----> cost == float("inf") 表示链路不可用
            self.name = name
...
class VirtualPacket:
    ...
    def __init__(self, src: Addr = None, des: Addr = None, payload: str = None, type=None,
        ...
        # 从参数初始化结构化的packet
            ...
            self.type = type  # 数据包类型，用于区分传播路由信息的数据包，如携带距离向量的数据包(type="dv")，和终端发送的携带数据的普通数据包(type="data")
            self.src = src
            self.des = des
            self.payload = payload  # 数据，任意字符串
